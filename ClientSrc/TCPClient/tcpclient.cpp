...
namespace uv
{
	...
	void TCPServer::AfterServerRecv(uv_stream_t *handle, intptr_t nread, const uv_buf_t* buf)
	{
		if (!handle->data) {
			return;
		}
	...
	void TCPClient::AfterClientRecv(uv_stream_t *handle, intptr_t nread, const uv_buf_t* buf)
	{
		if (!handle->data) {
			return;
		}
