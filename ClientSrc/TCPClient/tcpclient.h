...
protected:
    ...
    //静态回调函数
    static void AfterServerRecv(uv_stream_t *client, intptr_t nread, const uv_buf_t* buf);
...
protected:
    ...
    static void AfterClientRecv(uv_stream_t *client, intptr_t nread, const uv_buf_t* buf);
    static void AfterSend(uv_write_t *req, int status);
    static void onAllocBuffer(uv_handle_t *handle, size_t suggested_size, uv_buf_t *buf);
